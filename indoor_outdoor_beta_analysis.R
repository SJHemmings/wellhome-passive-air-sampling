...
METADATA <- 
  read.csv("Metadata/Wellhomes_metadata.csv") %>% as.tibble() %>% 
  rename_all(tolower) %>% 
...
ASV_COUNTS <- 
  read.csv("ASVs/All_runs/ASV_files/ASVs_counts.csv") %>% 
  as_tibble()

## Read in Taxonomy table
TAXONOMY <- 
  read_tsv("ASVs/All_runs/ASV_files/ASVs_taxonomy.tsv") %>% as.tibble() %>% select(-"...1") %>% 
...
ENV.dist <- readRDS("RDS_files/All_runs/avg_dis_all_runs.rds")
